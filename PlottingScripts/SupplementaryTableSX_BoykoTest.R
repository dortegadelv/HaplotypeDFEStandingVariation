...
DFESelection <- read.table("../Results/ResultsSelectionInferred/SelectionUK10KDFETest.txt")
...
LowerLimit <- c(LowerLimit,0.2297)
LowerLimit <- c(LowerLimit,2.297)
LowerLimit <- c(LowerLimit,7.297)
LowerLimit <- c(LowerLimit,12.297)
LowerLimit <- c(LowerLimit,17.297)
LowerLimit <- c(LowerLimit,22.97)

for (j in 1:100){
    SelectionDFERow <- ((DFESelection$V1[j] %% 52 ) + 1)
    SelectionDFEColumn <- (floor(DFESelection$V1[j] / 52 ) + 1)
    print (j)
    ...
    Probs <- P_Allele_Is_2Ns_given_OnePercent[1:6] * Prob_One_Percent
    
    # print (Probs)
    Probs <- Probs[1:6] / Probabilities_At_One_Percent_Given_2Ns[1:6]
    # print (Probs)
    if (sum(Probs) > 1.0){
        Probs[7] = 0
        Probs[1:6] <- Probs[1:6] / sum(Probs[1:6])
    }else{
        Probs[7] <- 1 - sum (Probs[1:6])
    }
    Probs[8] <- sum(Probs[3:6])
    P_Allele_Is_2Ns_given_OnePercent[7] <- 1 - sum(P_Allele_Is_2Ns_given_OnePercent)
    P_Allele_Is_2Ns_given_OnePercent[8] <- sum(P_Allele_Is_2Ns_given_OnePercent[3:6])
    # print (Probs)
    if (j==1){
