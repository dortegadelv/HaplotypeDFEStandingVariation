...
pdf("../Figures/SuppFigure11_DFEf_toDFEOn100Sims.pdf",width=10,height = 14)
...
for (i in 4:30){
counts <- cbind(counts,c(PartOne[i],PartTwo[i],PartThree[i],PartFour[i]))
}

counts <- cbind(counts,c(1-sum(PartOne[1:30]),1-sum(PartTwo[1:30]),1-sum(PartThree[1:30]),1-sum(PartFour[1:30])))
...
counts[3,1] <- PartThree[1]
counts[3,2] <- PartThree[2]
counts[3,3] <- PartThree[3]
counts[4,1] <- PartFour[1]
counts[4,2] <- PartFour[2]
counts[4,3] <- PartFour[3]

colnames(counts) <- c("0-5", "5-10", "10-15", "15-20", "20-25", "25-30", "30-35", "35-40", "40-45", "45-50","50-55", "55-60", "60-65", "65-70", "70-75", "75-80", "80-85", "85-90", "90-95", "95-100","100-105", "105-110", "110-115", "115-120", "120-125", "125-130", "130-135", "135-140", "140-145", "145-150", ">150")
rownames(counts) <- c("Real P(4Ns | DFE, D)", "Inferred P(4Ns| DFE, D) Equation 1","Inferred P(4Ns| DFE, D) Equation 2", "Inferred P(4Ns| 1%, DFE, D)")


barplot(counts, main="A) Constant Size - Human DFE", ylab="Probability", xlab="4Ns", col=ViridisColors,
legend = rownames(counts), beside=TRUE,cex.lab=2,cex.axis=2,cex.names=2, args.legend = list(x = "center",cex=2))
...
barplot(counts, main="B) Constant Size - Mouse DFE", ylab="Probability", xlab="4Ns", col=ViridisColors,
legend = rownames(counts), beside=TRUE,cex.lab=2,cex.axis=2,cex.names=2, args.legend = list(x = "center",cex=2))
...
plot(1:DivisionsPlusOne,c(P_Allele_Is_2Ns[1:Divisions], 1 - sum(P_Allele_Is_2Ns[1:Divisions])),col=ColorViridis[1],ylim=c(0,1.0),xaxt="n",ylab="Probability",xlab="4Ns",lwd=5,cex.lab=2,cex.axis=2,cex.lab=2, main = "C) Population expansion - Human DFE",cex.main = 2)
...
plot(1:DivisionsPlusOne,c(P_Allele_Is_2Ns[1:Divisions], 1 - sum(P_Allele_Is_2Ns[1:Divisions])),col=ColorViridis[1],ylim=c(0,1.0),xaxt="n",ylab="Probability",xlab="4Ns",lwd=5,cex.lab=2,cex.axis=2,cex.lab=2, main = "D) Population expansion - Mouse DFE",cex.main = 2)
