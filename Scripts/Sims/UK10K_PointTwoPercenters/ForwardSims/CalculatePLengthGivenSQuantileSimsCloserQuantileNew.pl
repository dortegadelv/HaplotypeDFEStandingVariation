...
# open (VARTEST,">ThisIsAVariantTest.txt") or die "NO!";
...
$QuantileLeft = $QuantTest;
if ($QuantTest != 0){
$DifCheck = abs($RecRate - $QuantileRec[$QuantTest])/$QuantileRec[$QuantTest];
if ($DifCheck < 0.005){
$SmallRecDifFlag = 1;
}else {
$SmallRecDifFlag = 0;
...
$DifCheck = abs($RecRate - $QuantileRec[$QuantTest+1])/$QuantileRec[$QuantTest+1];
if (($DifCheck < 0.005)){
$SmallRecDifFlag = 1;
}else {
$SmallRecDifFlag = 0;
...
if ( ( $RecRate < $QuantileRec[20] ) && (  $RecRate > $QuantileRec[0] ) && ($SmallRecDifFlag == 1) ){
...
# print VARTEST "$TotalVariantNumberUsed\t$RecRate\t$QuantileRec[$QuantileLeft]\tQL=$QuantileLeft\t$LogLikelihoods[$i]\t$ISValues[$QuantileLeft][$i+1][$j+1]\n";
$TotalVariantNumberUsed++;
}elsif ( $RecRate == $QuantileRec[0]){
$LogLikelihoods[$i] = $LogLikelihoods[$i] + log ($ISValues[0][$i+1][$j+1]);
# print VARTEST "$TotalVariantNumberUsed\t$RecRate\t$QuantileRec[$QuantileLeft]\tQL=0\t$LogLikelihoods[$i]\t$ISValues[$QuantileLeft][$i+1][$j+1]\n";
$TotalVariantNumberUsed++;
}elsif ( $RecRate == $QuantileRec[20]){
$LogLikelihoods[$i] = $LogLikelihoods[$i] + log ($ISValues[20][$i+1][$j+1]);
# print VARTEST "$TotalVariantNumberUsed\t$RecRate\t$QuantileRec[$QuantileLeft]\tQL=20\t$LogLikelihoods[$i]\t$ISValues[$QuantileLeft][$i+1][$j+1]\n";
$TotalVariantNumberUsed++;
}
...
print "Total number of variants used = $TotalVariantNumberUsed\n";
