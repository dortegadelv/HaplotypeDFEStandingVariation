...
def init_block_fun(mu, scalingfactor, es, shape, smin, smax, simseqinfoname, cnc=False):
    ...
    initialize() {{
        ...
        initializeMutationType("m1", 0, "g", {2}, {3});
...
def demog_block_fun_neu(nrep, Nanc, Tafnea, Nnea, Taf, Naf, Tb, Nb, mafb, Tadm,
    ...
    {2} early() {{
        sim.addSubpopSplit("p2", {3}, p1);
    }}

    // set African population size
    {4} early() {{
        p1.setSubpopulationSize({5});
    }}

    // split off Asian-Eur ancestor (p3)
    {6} early() {{
        sim.addSubpopSplit("p3", {7}, p1);
        p1.setMigrationRates(c(p2, p3), c(0, {8}));
        p3.setMigrationRates(c(p1, p2), c({8}, 0));
    }}

    // Neanderthal hybridization
    {9} late() {{
        sim.addSubpopSplit("p4", {7}, p1);
        p4.setMigrationRates(c(p1,p2,p3), c(0.0, {10}, {11}));
    }}

    {12} late() {{
        p4.setMigrationRates(c(p1,p2,p3), c({8}, 0.0, 0.0));
    ...
    {13} early() {{
        p4.setSubpopulationSize({14});
        p1.setMigrationRates(c(p4), c({15}));
        p4.setMigrationRates(c(p1), c({15}));
    }}

    // Asian exponential growth
    {13}:{16} {{
        newSize = asInteger(round({17}^(sim.generation - {13}) * {14}));
        p4.setSubpopulationSize(newSize);
    }}
    '''.format(rand_seeds, Nanc, Tafnea, Nnea, Taf, Naf, Tb, Nb, mafb,
               (Tadm-1), mafnea, (1.-mafnea), Tadm, Teuas, Nas0, mafas,
               Tcurr, ras, Tneasamp)
...
def main(args):
    ...
    Tcurr = Teuas + 920. #Current day time
...
if __name__ == "__main__":
    ...
    parser.add_argument('-z', '--size', action="store", dest="size",
                        help="size of chunk to simulate, default: 3100000",
                        default=3100000, type=int)
