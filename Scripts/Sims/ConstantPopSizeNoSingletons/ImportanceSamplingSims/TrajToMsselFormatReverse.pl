...
$NewTraj = $NewTraj."\n$SplitLine[1]\t$SplitLine[2]";
    push(@FreqTrajectory,$SplitLine[2]);
$NumberOfIntervals++;
$Age = $SplitLine[1];
}
elsif ($Line =~ "TOT"){
print EXIT "# s: 0.000000 age: $Age\n";
    $CurrentAge = 0;
    foreach $FrequencyValue ( reverse @FreqTrajectory){
        $ThisFrequency = $CurrentAge / (4 * $Ne);
        print EXIT "$ThisFrequency\t$FrequencyValue\n";
        $CurrentAge++;
