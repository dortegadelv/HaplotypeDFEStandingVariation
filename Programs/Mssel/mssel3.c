...
/*  Since the above bit is only done for count = 0, can't use tbs argument for argument of -s switch */
	...
	fscanf(pars.selfile," n: %d s: %lf age: %lf",&(pars.cp.nfreqder), &s, &age );
    ...
    fprintf(stdout,"rho: %lf theta: %lf N0: %d\n",pars.cp.r,pars.mp.theta, N0);
    ...
    while( howmany-count++ ) {
	   ...
	   if( (count>1)&& ( pars.cp.nrepsfreq >1) ){
		  oldnfreqder = pars.cp.nfreqder ;
	   	  if( fscanf(pars.selfile," n: %d s: %lf age: %lf",&(pars.cp.nfreqder), &s, &age ) != 3 ){
		     rewind(pars.selfile);
			 do {
                 fgets( line, 1000, pars.selfile );
             }while ( line[0] == '#' );

		     fscanf(pars.selfile," %*d %*d");
		     fscanf(pars.selfile," n: %d s: %lf age: %lf", &(pars.cp.nfreqder) , &s, &age) ;
          }
	   ...
	   if( ntbs >0 ){
        ...
        if( (segsites > 0 ) || ( pars.mp.theta > 0.0 ) ) {
   	       if( (pars.mp.segsitesin > 0 ) && ( pars.mp.theta > 0.0 )) 
		       fprintf(pf,"prob: %g\n", probss ) ;
			fprintf(pf,"selsite: %d s: %lf age: %lf\n", selsite, s, age);   /* sites numbered 0,1,..., segsites-1 */
           fprintf(pf,"segsites: %d\n",segsites);
		   if( segsites > 0 )	fprintf(pf,"positions: ");
		   for( i=0; i<segsites; i++)
              fprintf(pf,"%6.6lf ",posit[i] );
           fprintf(pf,"\n");
	       if( segsites > 0 ) 
...
getpars(int argc, char *argv[], int *phowmany )
{
  ...
  if( count == 0 ) {
	...
	*phowmany = atoi( argv[2] );
	...
	sscanf(line," %d N0: %d",&(pars.cp.nrepsfreq), &N0 );
