...
@DistanceFourNSPosSelProbabilities = ();
while (<TABLE>){
...
for ($k = 0; $k < scalar(@SplitLine) ; $k++){
$DistanceFourNSPosSelProbabilities[$k] = $SplitLine[$k];
}
}
$Number++;
}
...
open (PROBS, "AnotherExtraTableOfProbabilitiesPosBoykoSmall.txt" ) or die "NO!";
...
$CurrentValue = $CurrentValue + $Probabilities[$i][203] * $DistanceFourNSPosSelProbabilities[$k] ;
