...
double Currentf= f, PathXPrime, PathYPrime, FrequencyToPrint, AfterFrequency;
...
boost::normal_distribution<> nd(SampleFinalGenPopSize*f, sqrt(SampleFinalGenPopSize*f*(1-f)));
...
/*		Test = (int) ( bnldev(Currentf,Ne, &RepRandomNumber) + 0.5);
		...
		Currentf = double(CurrentAlleleNumber) / double(Ne); */
		Test = (int) ( (var_nor() / SampleFinalGenPopSize ) * Ne );
		CurrentAlleleNumber = Test;
		Currentf = double(CurrentAlleleNumber) / double(Ne);
		if (Test < 2){
