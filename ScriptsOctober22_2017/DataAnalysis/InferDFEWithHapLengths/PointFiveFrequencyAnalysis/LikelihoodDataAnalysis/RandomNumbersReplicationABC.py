...
## Number of variants = 310 ## Remember int(Ne*Fraction+0.5)
...
for CurrentRecRate in RecRates:
   ...
   GensOne = float(int ( TimeOne *Fraction+0.5 ))  / (4.0*float(int (NeInitial*Fraction+0.5 )))
   FractionOne = float(int (NeTwo*Fraction+0.5)  ) / float(int (NeInitial*Fraction+0.5 ))
   
#   GensTwo = TimeTwo / (4.0*float(NeInitial))
#   FractionTwo = NeThree / (float(NeInitial))
   
   GensBot =  float(int ( TimeTwo*Fraction+0.5 )) / (4.0*float(int (NeInitial *Fraction+0.5)))
   FractionBot = float ( int ( NeThree*Fraction+0.5)) / (float(int (NeInitial*Fraction+0.5)))
   
   GensFive = float( int (20000.0 *Fraction+0.5 )) / (4.0*float(int (NeInitial*Fraction+0.5)))
   FractionFive = float(int(12000 *Fraction+0.5)) / (float(int (NeInitial *Fraction+0.5 )))
