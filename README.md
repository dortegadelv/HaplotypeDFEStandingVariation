...
a. Model the uncertainty in our estimate of the population allele frequency
b. Evaluate multiple values of selection using the same set of simulated allele frequency trajectories.
...
3. ExamplePipeline.- This pipeline includes scripts that can be used to
    a) Generate the pairwise identity by state lengths L.
    b) Generate the table that computes the likelihoods of L(4Ns, allele frequency, Demographic scenario | L) for a single selection coefficient 4Ns (see equation 2 from our paper).
    c) Generate a table that computes the likelihoods of L(alpha, beta, allele frequency, Demographic scenario | L) for two parameters alpha and beta of a partially collapsed gamma distribution (see equation 3 from our paper).
    d) Compute the maximum likelihood estimate of either a) the single selection coefficient 4Ns or b) the two parameters alpha and beta.
    e) Estimate the DFE from DFEf.
    f) Calculate L from data.
    g) An ABC algorithm to estimate demographic history based on the L values at neutral sites.
