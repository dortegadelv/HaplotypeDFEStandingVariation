...
par(mar=c(5,5,4,2) + 0.1)   
	...
	Color <- col2rgb("black")
	if (j==7 || j==8){
		plot(Data$V1[5000:1],Data$V2[1:5000],xlab="Generations before the present",ylab="Mean Allele Frequency",type="l",xlim=c(XLimFigureOne[j],5000),ylim=c(0,ListMaxFreq[j]*1.3),main="A) Average Frequency Trajectory",xaxt="n",cex.lab=2,cex.main=2,cex.axis=2,lwd=6,col=ColorViridis[3])
		
	}else{
		plot(Data$V1[5000:1],Data$V2[1:5000],xlab="Generations before the present",ylab="Mean Allele Frequency",type="l",xlim=c(XLimFigureOne[j],5000),ylim=c(0,ListMaxFreq[j]),main="A) Average Frequency Trajectory",xaxt="n",cex.lab=2,cex.main=2,cex.axis=2,lwd=6,col=ColorViridis[3])
	}
...
#	XLimFigureTwo[11]=3000
	if (XLimFigureTwo[j] <= 1000){
		axis(1,at=c(max(density(Data$V1*Ne[j]*2,from=0,to=Max)$x)-XLimFigureTwo[j] + 0.1*(XLimFigureTwo[j]),Max-500,Max-250,Max),labels=c(ValueToPrintXAxis,"500","250","Present"),cex.lab=2,cex.axis=2)
...
for (j in 1:1){
	...
	Plot <- paste("PotentialPaperFigures/ThreePanelsNew",DemScenario[j],".pdf",sep="")
	pdf(Plot,width=12,height=12)
	par(mfrow=c(2,2))
	par(mar=c(5,5,4,2) + 0.1)   
