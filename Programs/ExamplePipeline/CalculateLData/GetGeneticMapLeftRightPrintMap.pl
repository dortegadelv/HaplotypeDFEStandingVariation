...
open (PERRECVAR,">../Results/LeftBpRecRatePerVariantPrintMap.txt") or die "NO!";

$Bound = $ARGV[0];
# $FrequencyFilePrefix = "MissenseOnePercent";
$FrequencyFilePrefix = $ARGV[1];
$MapFilePrefix= $ARGV[2];
$Chromosome = $ARGV[3];
$TotalLineNumber = -1;
for ($i = $Chromosome; $i <= $Chromosome; $i++){

$GeneticMapFile = $FrequencyFilePrefix.$i.".frq";
...
# print "$Position[1]\n";
$ActualMap = $MapFilePrefix.$i;

$LowerPosition = $Position[1] - $Bound;
...
$TotalRecRate = (($LastPosition - $FirstPosition + $GenMapFirstPart + $GenMapLastPart ) / $Bound) / 100;
print PERRECVAR "$UpperPosition\t$TotalRecRate\n";
print  "$UpperPosition\t$TotalRecRate\n";
...
open (PERRECVAR,">../Results/RightBpRecRatePerVariantPrintMap.txt") or die "NO!";
...
open (MAP,$GeneticMapFile) or die "Cannot open $GeneticMapFile\n";
...
$UpperPosition = $Position[1] + $Bound;
...
# print PERRECVAR "$FirstPosition\t$LastPosition\t$TotalRecRate\t$GenMapFirstPart\t$GenMapLastPart\n";
# print PERRECVAR "$TotalRecRate\n";
# die "No!\n";
close (ACTMAP);
#print "$GenMapSum\n";
